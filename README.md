...
Every change carries a stable `id` (`c` + a 4+ hex-char slice of the slug's sha1 hash, e.g. `c4f2a`; longer only for the rare slug that collides with another on the same board) and a `group` marking its parallel-dependency wave (`g001`, `g002`, … by longest dependency-chain depth; `!` when a blocker is unresolved — a dangling dependency or a `dep-cycle`; `—` for `done` or `completed` changes). The `id` depends only on the slug, so adding, removing, or archiving other changes never changes an existing change's id — unlike `group`, which is a live value expected to shift as work completes. Two changes sharing a group have no dependency path between them and can run in parallel, even across sprint boundaries. Blockers are reported by `id`, not slug.
...
Each sprint (and the `Ungrouped` list, and the trailing `Warnings` table) renders as a left-railed panel with its title bolded and underlined, so a reader can see where one section ends and the next begins. Every change table shares one column grid — `status`/`group`/`blockers`/`gaps`/`missing`/`title` start at the same offset in every panel — and columns shrink together, widest first, to fit the terminal width (or 120 columns when not a TTY) instead of each panel sizing itself independently. Rows for satisfied changes (`done`, `completed`) are struck through; rows that are merely blocked are dimmed instead — a satisfied change that still carries a stale blocker is struck through, not dimmed, since there is nothing left to act on. The rail itself is structural and always renders, including under `--plain`; only the bold/underline/strikethrough/dim decorations are gated. Pass `--plain` to force plain output regardless of terminal detection; color is otherwise skipped automatically when stdout is not a TTY, and the `NO_COLOR` env var disables it too. `--json` output never carries ANSI codes.

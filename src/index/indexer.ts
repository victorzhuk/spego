...
import { artifactFilePath, type WorkspacePaths } from '../workspace/paths.js';
import { parseArtifactFile } from '../artifacts/storage.js';
...
function formatIndexError(err: unknown): string {
 return err instanceof SpegoError ? `${err.code}: ${err.message}` : (err as Error).message;
}
...
export function getArtifactByTypeSlug(db: Db, type: string, slug: string): IndexedArtifact | null {
 const row = db.prepare(`SELECT * FROM artifacts WHERE type = ? AND slug = ?`).get(type, slug) as
  | Record<string, unknown>
  | undefined;
 return row ? rowToArtifact(row) : null;
}
...
export async function rebuildIndex(
 ...
 for (const type of typeDirs) {
  ...
  let stat;
  try {
   stat = await fs.lstat(typeDir);
  } catch {
   continue;
  }
  if (stat.isSymbolicLink()) {
   report.invalidFiles.push({ path: typeDir, error: 'SYMLINK: type directory is a symlink' });
   continue;
  }
  ...
  for (const entry of entries) {
   ...
   let entryStat;
   try {
    entryStat = await fs.lstat(file);
   } catch {
    report.invalidFiles.push({ path: file, error: 'STAT_FAILED: cannot stat file' });
    continue;
   }
   if (entryStat.isSymbolicLink()) {
    report.invalidFiles.push({ path: file, error: 'SYMLINK: artifact file is a symlink' });
    continue;
   }
   ...
   try {
    ...
    if (knownTypes && !knownTypes.has(frontmatter.type)) {
     throw new SpegoError(
      ...
      `Unsupported artifact type: ${frontmatter.type}`,
      {
    ...
    const message = formatIndexError(err);
 ...
 for (const entry of entries) {
  ...
  try {
   ...
   try {
    ...
    report.invalidFiles.push({ path: file, error: 'STAT_FAILED: cannot stat revision' });
    continue;
   }
   if (entryStat.isSymbolicLink()) {
    report.invalidFiles.push({ path: file, error: 'SYMLINK: revision file is a symlink' });
    continue;
   }
   const text = await fs.readFile(file, 'utf8');
   const { frontmatter } = parseArtifactFile(text, file);

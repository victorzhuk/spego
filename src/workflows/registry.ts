...
const BRAINSTORM_PARTY: WorkflowMeta = {
  ...
  phases: [
    ...
    { name: 'record', instruction: 'Persist the brainstorm results by running: spego --json create --type brainstorm --title "<topic> brainstorm" --body "<converged output as markdown>". Optionally follow up with spego --json create --type prd or spego --json create --type design for the top-ranked idea. Capture the returned `id` so you can chain follow-ups.' },
  ],
  inputs: [
...
const REVIEW_ADVERSARIAL: WorkflowMeta = {
  ...
  phases: [
    { name: 'read', instruction: 'Read the source artifact by running: spego --json read --id <artifactRef> (or spego --json read --type <type> --slug <slug>). Every persona reads the full content silently before attacking.' },
    ...
    { name: 'record', instruction: 'Persist the review results by running: spego --json create --type qa --title "<artifact name> review" --body "<prioritized findings>" OR spego --json create --type risk --title "<artifact name> risks" --body "<risk findings>". Optionally follow up with spego --json create --type decision or spego --json create --type retro. Capture the returned `id` so you can chain follow-ups.' },
  ],
  inputs: [
...
const REVIEW_EDGE_CASES: WorkflowMeta = {
  ...
  phases: [
    { name: 'read', instruction: 'Read the source artifact by running: spego --json read --id <artifactRef> (or spego --json read --type <type> --slug <slug>). Every persona reads the full content silently before analysis.' },
    ...
    { name: 'record', instruction: 'Persist findings by running: spego --json create --type qa --title "<artifact name> edge-case review" --body "<markdown containing a fenced json block>". The JSON block must be an array of findings, each with fields: location (section/line reference), trigger_condition (what causes this path), guard_snippet (existing guard if any, or "none"), and potential_consequence (what goes wrong). Only include unhandled cases.' },
  ],
  inputs: [
    { name: 'artifactRef', required: true, description: 'Artifact id, or type:slug reference, of the artifact to review.' },
...
const EDITORIAL_PROSE: WorkflowMeta = {
  ...
  phases: [
    { name: 'read', instruction: 'Read the source artifact by running: spego --json read --id <artifactRef> (or spego --json read --type <type> --slug <slug>). The Copy-editor reads the full content silently.' },
    ...
    { name: 'apply-or-recommend', instruction: 'For each proposed edit: (a) present the diff to the user with the reason, (b) ask for confirmation, (c) if confirmed, apply it by running: spego --json update --id <source artifact id> --expected-revision <current revision> --body "<updated content>". If the user declines or is unsure, mark it as "recommended only". Always pass --expected-revision to catch concurrent edits. Capture the new `revision` from the JSON response for the next edit. Record both applied and recommended-only edits.' },
    { name: 'record', instruction: 'Persist the edit summary by running: spego --json create --type qa --title "<artifact name> prose edit summary" --body "<markdown listing all edits: applied edits with their diffs and recommended-only edits with their diffs>". This creates a traceable record of what changed and what was recommended but not applied.' },
  ],
  inputs: [
...
const EDITORIAL_STRUCTURE: WorkflowMeta = {
  ...
  phases: [
    { name: 'read', instruction: 'Read the source artifact by running: spego --json read --id <artifactRef> (or spego --json read --type <type> --slug <slug>). The Structural editor reads the full content silently.' },
    ...
    { name: 'record', instruction: 'Persist the restructuring recommendations by running: spego --json create --type qa --title "<artifact name> structural review" --body "<prioritized restructuring proposals as markdown>". This workflow does not mutate the source artifact — it only produces recommendations.' },
  ],
  inputs: [
...
const BRAINSTORM_DEEP: WorkflowMeta = {
  ...
  phases: [
    { name: 'frame', instruction: 'State the topic clearly. Define scope, constraints, and what counts as a valid idea. If a `seedArtifactId` was provided, run `spego --json read --id <seedArtifactId>` and extract relevant context to frame the brainstorm. Set the target idea count (default: 50, or the user-specified `target_count`).' },
    ...
    { name: 'record', instruction: 'Persist the brainstorm results by running:\n```\nspego --json create --type brainstorm --title "<topic> brainstorm" --body "<full output: raw ideas, clusters, and top-10 ranking>"\n```\nOptionally follow up with `spego --json create --type prd` for the top-ranked idea. Capture the returned `id` from each call.' },
  ],
  inputs: [

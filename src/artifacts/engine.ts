...
import { closeIndexDb, ensureIndexSchema, openIndexDb, type Db } from '../index/db.js';
import {
 getArtifactById,
...
import { atomicWriteFile, readArtifactFile, renderArtifactFile } from './storage.js';
...
export class ArtifactEngine {
 ...
 renderMarkdown(record: ArtifactRecord): string {
 ...
 async create(input: unknown): Promise<ArtifactRecord> {
  ...
  const snapPath = await writeRevisionSnapshot(this.paths, fm, data.body, this.resolvedRoot);
  ...
  if (
   ...
   throw new SpegoError(
    ...
    `Stale revision for ${id}: expected ${data.expectedRevision}, current ${current.frontmatter.revision}`,
    {
     ...
     currentRevision: current.frontmatter.revision,
    },
   );
  }
 ...
 async softDelete(id: string): Promise<ArtifactRecord> {
  ...
  await atomicWriteFile(indexed.path, renderArtifactFile(fm, current.body), this.resolvedRoot);
  const snapPath = await writeRevisionSnapshot(this.paths, fm, current.body, this.resolvedRoot);
  upsertArtifact(this.db, fm, indexed.path, snapPath);
 ...
 async readRevision(id: string, revision: number): Promise<ArtifactRecord> {
  ...
  const snap = await readRevisionSnapshot(this.paths, indexed.type, indexed.slug, revision);

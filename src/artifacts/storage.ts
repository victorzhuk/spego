...
import { assertParentContained } from '../workspace/paths.js';
...
export function parseArtifactFile(
  ...
  if (!openMatch) {
    throw new SpegoError('VALIDATION_FAILED', `Missing frontmatter block in ${sourcePath}`, {
      path: sourcePath,
    });
  }
  ...
  if (!closeMatch || closeMatch.index === undefined) {
    throw new SpegoError('VALIDATION_FAILED', `Unterminated frontmatter block in ${sourcePath}`, {
      path: sourcePath,
    });
  }
  ...
  try {
    ...
    throw new SpegoError('VALIDATION_FAILED', `Invalid YAML frontmatter in ${sourcePath}`, {
      path: sourcePath,
...
/** Read and parse a markdown artifact file from disk, rejecting symlinked targets. */
export async function readArtifactFile(absPath: string): Promise<{
  ...
  if (stat.isSymbolicLink()) {
    throw new SpegoError('WORKSPACE_CONTAINMENT', `Symlinked artifact file at ${absPath}`, {
      path: absPath,
    });
  }
  let text: string;
 ...
 * Verifies containment when `resolvedRoot` is provided.
 */
export async function atomicWriteFile(
  ...
  if (resolvedRoot) {
    await assertParentContained(resolvedRoot, absPath);
  }
